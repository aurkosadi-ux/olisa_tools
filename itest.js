...
t('a missed challan gets a diagnosis, not a dead end', /function showChallanMiss/.test(html));
t('every scanned document is logged with its outcome', /challanFileLog/.test(html) && /no-challan-number/.test(html));
t('the file log survives a reload with the index', /cached\.files \|\| \[\]/.test(html));
t('a miss falls back to searching file names', /\(\^\|\[\^0-9\]\)0\*\$\{key\}/.test(html));
t('unreadable documents can be opened straight from the miss', /class="ch-file"/.test(html));
t('Share no longer swallows failures silently', !/navigator\.share\([^)]*\)\.catch\(\(\) => \{\}\)/.test(html));
t('Share falls back to a download', /UnsupportedError/.test(html) && /doDownload\(\);/.test(html));
t('a cancelled share stays quiet', /err\.name === 'AbortError'/.test(html));
t('files get a real MIME type for the share sheet', /application\/msword/.test(html) && /application\/rtf/.test(html));
t('the Share button is never hidden', !/shareBtn\.style\.display = 'none'/.test(html));

console.log('\n8. Service worker');
const sw = fs.readFileSync(process.argv[3] || 'sw.js', 'utf8');
const ver = (sw.match(/olisa-tools-v(\d+)/) || [])[1];
t('cache version bumped past v31', ver && Number(ver) > 31, 'v' + ver);
